      ...
      <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black bg-opacity-50 p-4">
        <div className={`rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-fade-in flex flex-col ${isHospitalMode ? 'bg-[#212327]' : 'bg-white'}`}>
           <div className="p-8 text-center space-y-4">
              ...
              <h3 className={`text-xl font-bold ${isHospitalMode ? 'text-white' : 'text-gray-900'}`}>Agendamento Confirmado!</h3>
              <p className={`text-sm ${isHospitalMode ? 'text-gray-400' : 'text-gray-500'}`}>Visita para <strong>{new Date(selectedDate + 'T12:00:00').toLocaleDateString()}</strong> na rota <strong>{selectedRoute.name}</strong>.</p>
              ...
              <div className="pt-4 space-y-2">
                ...
                <button onClick={onClose} className={`w-full py-3 font-bold text-sm ${isHospitalMode ? 'text-gray-500 hover:text-gray-300' : 'text-gray-400 hover:text-gray-600'}`}>Voltar para Agenda</button>
              </div>
           </div>
        </div>
      </div>
      ...
      <div className={`rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in flex flex-col max-h-[90vh] ${isHospitalMode ? 'bg-[#212327]' : 'bg-white'}`}>
        ...
        <div className="bg-blue-600 px-6 py-5 flex justify-between items-center flex-shrink-0">
          <div>
            ...
            <p className="text-blue-100 text-xs uppercase tracking-widest font-bold">Passo {step} de 2</p>
          </div>
        ...
        <div className="p-6 flex-grow overflow-y-auto custom-scrollbar">
          {step === 1 && (
             <div className="space-y-6 animate-fade-in">
                <div>
                   <label className={`block text-[10px] font-bold uppercase mb-2 ${isHospitalMode ? 'text-gray-500' : 'text-gray-400'}`}>1. Data da Visita</label>
                   <input 
                     type="date" 
                ...
                <div>
                   <label className={`block text-[10px] font-bold uppercase mb-3 ${isHospitalMode ? 'text-gray-500' : 'text-gray-400'}`}>2. Selecione a Rota Disponível</label>
                          ...
                          return (
                              ...
                              >
                                 <div className="flex-grow">
                                    ...
                                    <p className="text-[10px] text-gray-400 mt-1 truncate max-w-[200px]">{route.hospitals.join(' • ')}</p>
                                 </div>
          ...
          {step === 2 && selectedRoute && (
              <div className="space-y-6 animate-fade-in">
                  <div className={`p-4 rounded-xl border-2 ${isHospitalMode ? 'bg-blue-900/10 border-blue-900/40' : 'bg-blue-50 border-blue-100'}`}>
                      <p className={`text-[9px] font-bold uppercase mb-1 ${isHospitalMode ? 'text-blue-400' : 'text-blue-600'}`}>Resumo da Escala</p>
                      <p className={`font-bold text-base ${isHospitalMode ? 'text-gray-100' : 'text-gray-800'}`}>{selectedRoute.name}</p>
                  ...
                  <div>
                     <label className={`block text-[10px] font-bold uppercase mb-3 ${isHospitalMode ? 'text-gray-500' : 'text-gray-400'}`}>3. Formar a Dupla ({tempMemberIds.length}/2)</label>
                            ...
                            return (
                                ...
                                >
                                   <div className="flex items-center gap-3">
                                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold ${isSelected ? 'bg-blue-600 text-white shadow-sm' : 'bg-gray-700 text-gray-300'}`}>
                                          {member.name.substring(0, 2).toUpperCase()}
                                      </div>
                                      <div>
                                          <p className={`text-sm font-bold ${isHospitalMode ? 'text-gray-200' : 'text-gray-800'}`}>{member.name} {isMe && '(Você)'}</p>
                                          {!canToggle && <p className="text-[9px] text-red-500 font-bold uppercase">Apenas Admins</p>}
                                      </div>
                                   </div>
        ...
        <div className={`p-6 border-t flex justify-between gap-3 ${isHospitalMode ? 'bg-[#1a1c1e] border-gray-800' : 'bg-white border-gray-100'}`}>
          {step === 1 ? (
              <>
                ...
                <Button className="w-full" disabled={!selectedRouteId} onClick={() => setStep(2)}>Próximo Passo</Button>
              </>
          ) : (
              <>
                <Button variant="ghost" className="w-full" onClick={() => setStep(1)}>Voltar</Button>
                <Button className="w-full" onClick={handleConfirm} disabled={tempMemberIds.length === 0}>Confirmar Visita</Button>
              </>
          )}
        </div>
      </div>
