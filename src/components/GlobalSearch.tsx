...
export const GlobalSearch: React.FC<GlobalSearchProps> = ({ state }) => {
  ...
  useEffect(() => {
    ...
    // 2. Search Patients (Active & Inactive)
    ...
    // 3. Search Routes/Hospitals (Definitions)
  ...
  const handleSelect = (result: SearchResult) => {
    ...
    if (result.type === 'member') {
      ...
      navigate('/dashboard'); // Just go to dashboard main view
    }
  };
  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-md hidden md:block mx-4">
      ...
      {isOpen && results.length > 0 && (
        <div className="absolute z-50 mt-1 w-full bg-white shadow-lg rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto max-h-96 focus:outline-none sm:text-sm">
          {results.map((result) => (
            <div
