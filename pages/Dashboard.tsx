...
export const Dashboard: React.FC<DashboardProps> = ({ state, onUpdateState, isPrivacyMode, isHospitalMode }) => {
  ...
  const handleOpenMyVisit = (visit: VisitSlot) => {
      ...
      if (route) {
          setMyVisitModalData({ slot: visit, route });
      }
  };
    ...
    <div className="space-y-6 pb-20 animate-fade-in">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <KpiCard 
                ...
                icon={<svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>}
        ...
        {/* MEUS AGENDAMENTOS (VISITAS REGULARES GVP) */}
                    ...
                    {myUpcomingVisits.map(visit => {
                        ...
                        return (
                            <button
                                ...
                                className={`text-left p-4 rounded-xl backdrop-blur-md border transition-all active:scale-95 group ${
                                    isHospitalMode 
                            ...
                            >
                                <div className="flex justify-between items-start">
                                    <div>
                                        <p className={`font-bold text-lg ${isHospitalMode ? 'text-white' : 'text-white'}`}>{route?.name}</p>
                                        <p className={`text-xs mt-1 ${isHospitalMode ? 'text-gray-300' : 'text-blue-100'}`}>
                                    ...
                                    <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${
                                        ...
                                        : (isHospitalMode ? 'bg-gray-700 text-gray-300' : 'bg-blue-800/50 text-blue-100')
                                    }`}>
                                        {new Date(visit.date + 'T12:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}
                                    </span>
                                </div>
                                <div className="mt-3 flex items-center gap-2 text-xs font-medium opacity-80 group-hover:opacity-100 transition-opacity">
                                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                                    Dupla: {partner ? partner.name : 'Aguardando parceiro'}
                                </div>
                            </button>
                        );
                    })}
